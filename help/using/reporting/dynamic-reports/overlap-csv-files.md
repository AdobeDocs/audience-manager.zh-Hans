---
description: 当报表达到 100 万条记录上限时，您可以为重叠报表申请一个 .csv 文件。当您看到“发生意外错误”消息时，报告可能已达到此限制。 请联系客户关怀部门以请求压缩的。csv文件，您可以导入该文件并在自己的数据库系统中使用它。 文件可用于段到段、段到特征和特征到特征重叠报表。
seo-description: 当报表达到 100 万条记录上限时，您可以为重叠报表申请一个 .csv 文件。当您看到“发生意外错误”消息时，报告可能已达到此限制。 请联系客户关怀部门以请求压缩的。csv文件，您可以导入该文件并在自己的数据库系统中使用它。 文件可用于段到段、段到特征和特征到特征重叠报表。
...
当报表达到 100 万条记录上限时，您可以为重叠报表申请一个 .csv 文件。当您看到“发生意外错误”消息时，报告可能已达到此限制。 请联系客户关怀部门以请求压缩的。csv文件，您可以导入该文件并在自己的数据库系统中使用它。 文件可用于段到段、段到特征和特征到特征重叠报表。

## 文件名元数据 {#file-name-metadata}

下表列出并描述了重叠。csv文件中使用的文件命名惯例和文件扩展名。 In the examples, *italics* indicates a variable placeholder.
...
<table id="table_C99FCABA365B4AB99620F27D4414E623"> 
 ...
 <tbody> 
  <tr> 
   ...
   <td colname="col2"> <p>重叠的报告文件经过gzip压缩，并具有 <code> .gz</code> 文件扩展名。 解压缩后，必须 <code> 将。csv</code> extension添加到文件。 </p> </td> 
  </tr> 
  <tr> 
   <td colname="col1"> <p>文件名 </p> </td> 
   <td colname="col2"> <p>文件名语法： </p> <p> 
     <ul id="ul_D69D320A1AE94361B75D2AB47F90C4D1"> 
      <li id="li_FFB104975D104050AB67FEEC903C6E2E">段到段文件： <code>S2S_overlap_<i>partner ID</i>_<i>yyyy-mm-dd</i>_date范<i>围</i></code> </li> 
      <li id="li_7DEC51D693FB4377840D652AF40386EF">段到特征文件： <code>S2T_overlap_<i>partner ID</i>_<i>yyyy-mm-dd</i>_date范<i>围</i></code> </li> 
      <li id="li_CCB35A2BCB714E518AB279D453740623">特征到特征文件： <code>T2T_overlap_<i>partner ID</i>_<i>yyyy-mm-dd</i>_date范<i>围</i></code> </li> 
     </ul> </p> </td> 
  </tr> 
  <tr> 
   <td colname="col1"> <p>日期范围 </p> </td> 
   <td colname="col2"> <p>报表的日期范围是5位数的ID，包括： </p> <p> 
     <ul id="ul_7B334CDFD7DA42AC8F383BE0F8F77FB9"> 
      <li id="li_0045DED532E747C08824DCC98A9174B3"> <code> 70000</code> ,7天报告。 </li> 
      <li id="li_3A22775F78E648BF8AC32A9E1AF1F5DE"> <code> 30000</code> ,30天报告。 </li> 
     </ul> </p> </td> 
  </tr> 
  <tr> 
   <td colname="col1"> <p>多个文件 </p> </td> 
   <td colname="col2"> <p>如果报表包含多个文件，我们会增加文件名中的最后一位。 </p> </td> 
  </tr> 
  <tr> 
   <td colname="col1"> <p>示例 </p> </td> 
   <td colname="col2"> <p>单个报告的文件名示例： </p> <p> 
     <ul id="ul_EED13F73F37D48868236F8945E19C88F"> 
      <li id="li_55DD677F9BA7460AA4AAD27AFD08A5AE">单日文件： <code> S2S_overlap_12345_2017_01_14_70000.gz</code> </li> 
...
在文件中，字符串数据用双引号括起来。 请参阅下面的模型数据。 为了简便和适合屏幕，已截断该选项。
...
## 细分到细分的报告记录 {#segment-segment-records}

“区段到区段重叠 [报告”的数据文件包含](segment-segment-overlap-report.md) 以下记录。
...
<table id="table_1BDC7019DF2543069D7AE229C5E2454E"> 
 ...
 <tbody> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p>基线区段的ID。 基线区段是要与其他区段进行比较的区段。 </p> </td> 
  </tr> 
  <tr> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p>您可以获得7天和30天回顾间隔的报告。 rangeid <code></code> 对应于下面显示的时间间隔。 </p> <p> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p>与基准区段比较的区段中的唯一用户数。 </p> </td> 
  </tr> 
  <tr> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p>基准区段与选定进行比较的其他区段之间唯一用户重叠的总数。 </p> </td> 
  </tr> 
  <tr> 
   <td colname="col1"> <p> <code> Overlap_perc</code> </p> </td> 
   <td colname="col2"> <p>基准区段与选定进行比较的其他区段之间唯一用户的重叠百分比。 </p> </td> 
  </tr> 
 </tbody> 
</table>

## 段到特征报表记录 {#segment-trait-records}

段到特征重叠报 [表的数据文件包含](segment-trait-overlap-report.md) 以下记录。
...
<table id="table_45270B5D01014AD99921B320D3A32DB6"> 
 ...
 <tbody> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p>数据提供者的名称。 </p> </td> 
  </tr> 
  <tr> 
   <td colname="col1"> <p> <code> rangeid</code> </p> </td> 
   <td colname="col2"> <p>您可以获得7天和30天回顾间隔的报告。 rangeid <code></code> 对应于下面显示的时间间隔。 </p> <p> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p>选定区段中的唯一用户数。 </p> </td> 
  </tr> 
  <tr> 
   <td colname="col1"> <p> <code> trait_uniques</code> </p> </td> 
   <td colname="col2"> <p>特征中的唯一用户数。 在UI报告中，当您将鼠标悬停在热图结果中的某个特征上时，该数字会显示在弹出窗口中。 </p> </td> 
  </tr> 
  <tr> 
   <td colname="col1"> <p> <code> overlap_uniques</code> </p> </td> 
   <td colname="col2"> <p>在选定区段和特征之间共享的唯一用户数。 在UI报告中，当您将鼠标悬停在热图结果中的某个特征上时，该数字会显示在弹出窗口中。 </p> </td> 
  </tr> 
  <tr> 
   <td colname="col1"> <p> <code> trait_uniques_overlap_perc</code> </p> </td> 
   <td colname="col2"> <p>特征和区段之间重叠的唯一用户的百分比。 在UI报告中，当您将鼠标悬停在热图结果中的某个特征上时，该数字会显示在弹出窗口中。 </p> </td> 
  </tr> 
  <tr> 
   <td colname="col1"> <p> <code> segment_uniques_overlap_perc</code> </p> </td> 
   <td colname="col2"> <p>区段和特征之间重叠的唯一用户百分比。 在UI报告中，当您将鼠标悬停在热图结果中的某个特征上时，该数字会显示在弹出窗口中。 </p> </td> 
  </tr> 
 </tbody> 
</table>
...
“特征到特征重 [叠”报告的数据文件包含](trait-trait-overlap-report.md) 以下记录。
...
<table id="table_603216E6AFE4439A87C91DDFF2989F53"> 
 ...
 <tbody> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p>在所选特征之间共享的唯一用户数。 在UI报告中，当您将鼠标悬停在热图结果中的某个特征上时，该数字会显示在弹出窗口中。 </p> </td> 
  </tr> 
  <tr> 
   <td colname="col1"> <p> <code> base_trait_uniques</code> </p> </td> 
   <td colname="col2"> <p>基本特征中的唯一用户数。 在UI报告中，当您将鼠标悬停在热图结果中的某个特征上时，该数字会显示在弹出窗口中。 </p> </td> 
  </tr> 
  <tr> 
   <td colname="col1"> <p> <code> overlap_uniques</code> </p> </td> 
   <td colname="col2"> <p>在所选特征之间共享的唯一用户数。 在UI报告中，当您将鼠标悬停在热图结果中的某个特征上时，该数字会显示在弹出窗口中。 </p> </td> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p>在所选特征之间重叠的唯一用户百分比。 在UI报告中，当您将鼠标悬停在热图结果中的某个特征上时，该数字会显示在弹出窗口中。 </p> </td> 
  </tr> 
  <tr> 
