...
| 主题 | 描述 |
|----|----|
| [Predictive Audiences](/help/using/features/algorithmic-models/predictive-audiences.md) | <ul><li>**预测受众模型克隆**:您现在可以克隆现有模型并根据需要更改其配置。 读取 [克隆和编辑预测受众模型](/help/using/features/algorithmic-models/predictive-audiences.md#clone-predictive-audiences) 以了解更多信息。</li><li>**预测受众批量分类**:除了以不同角色形式对访客进行实时分类之外，预测受众现在还支持批量分类，可以帮助您对已载入的用户进行分类并激活到批量目标。</li></ul> |
| [重叠报表](/help/using/reporting/dynamic-reports/dynamic-reports.md) | 我们更新了计算重叠报表的方法([特征到特征](/help/using/reporting/dynamic-reports/trait-trait-overlap-report.md), [区段到特征](/help/using/reporting/dynamic-reports/segment-trait-overlap-report.md), [区段到区段](/help/using/reporting/dynamic-reports/segment-segment-overlap-report.md))。 现在，重叠报表的计算是基于报表独特性和MinHash签名(而不是 [1/54数据采样](/help/using/reporting/report-sampling.md#data-sampling-ratio))。 |
...
| [入站S3文件名](/help/using/integration/sending-audience-data/batch-data-transfer-explained/inbound-s3-filenames.md) 和 [入站FTP文件名](/help/using/integration/sending-audience-data/batch-data-transfer-explained/inbound-ftp-filenames.md) | 我们更新了定义，并提供了 `DPID` 和 `DPID_TARGET_DATA_OWNER` 字段。 |

## 2020年8月 {#aam-august-updates}

| 主题 | 描述 |
|---- |----|
| [Google客户匹配（针对基于人员的目标）](../features/destinations/people-based-destinations-prerequisites.md#gcm) | 您现在可以使用 [!UICONTROL People-Based Destinations] 通过 [!DNL Google Customer Match]. 在使用之前 [!UICONTROL People-Based Destinations] 将第一方受众区段发送到 [!DNL Google Customer Match] 目标，则必须 [!DNL Google] 将您添加到其允许列表。 检查 [基于人员的目标](../features/destinations/people-based-destinations-prerequisites.md#gcm) 文档以了解详细信息。 |
| [新的SFTP服务器](../faq/faq-data-collection.md) | 您现在可以使用 [!DNL SFTP] 服务器([!DNL ftp-in-gtw] 和 [!DNL ftp-out-gtw]) [!DNL AWS SFTP Gateway] over [!DNL S3]. |
...
| 主题 | 描述 |
|---- |----|
| [在 Audience Manager 与 Adobe Experience Platform 之间共享受众](/help/using/integration/integration-aep/aam-aep-audience-sharing.md#aep-aam-segment-population-differences) | 新章节， **了解Audience Manager和Experience Platform之间的区段群体差异**，描述群体数字因Experience Platform和Audience Manager而异的可能原因。 |
| [开始使用Audience ManagerREST API](/help/using/api/rest-api-main/aam-api-getting-started.md#jwt) | 我们增加了新章节， **使用的JWT（服务帐户）身份验证[!DNL Adobe I/O]**. 请参阅此部分，以使用对Audience ManagerAPI进行身份验证 [!DNL Adobe I/O]. |
...
| [如何将数据从 Adobe Analytics 转发到 Audience Manager](../integration/integration-other-solutions/audience-management-module.md) | 我们更新了本页，以突出显示Adobe Experience Platform标记，这是将数据从Adobe Analytics转发到Audience Manager的首选方式。 |
