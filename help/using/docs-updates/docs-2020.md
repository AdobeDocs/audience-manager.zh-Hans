...
| 主题 | 描述 |
|----|----|
| [Predictive Audiences](/help/using/features/algorithmic-models/predictive-audiences.md) | <ul><li>**预测受众模型克隆**:您现在可以克隆现有模型并根据需要更改其配置。 阅读 [仿制和编辑预测受众模型](/help/using/features/algorithmic-models/predictive-audiences.md#clone-predictive-audiences) ，了解更多信息。</li><li>**预测受众批分类**:除了实时将不同角色的访客分类外，Predictive受众现在还支持批量分类，帮助您对已载入的用户进行分类并激活到批量目标。</li></ul> |
| [重叠报告](/help/using/reporting/dynamic-reports/dynamic-reports.md) | 我们更新了用于计算重叠报表([特征到特征](/help/using/reporting/dynamic-reports/trait-trait-overlap-report.md)[、区段到特征、](/help/using/reporting/dynamic-reports/segment-trait-overlap-report.md)区段到区段)的方法 [](/help/using/reporting/dynamic-reports/segment-segment-overlap-report.md)。 重叠报表计算现在基于报告唯一值和MinHash签名(而 [不是1/54数据采样](/help/using/reporting/report-sampling.md#data-sampling-ratio))。 |
...
| [入站S3文件名](/help/using/integration/sending-audience-data/batch-data-transfer-explained/inbound-s3-filenames.md) 和入 [站FTP文件名](/help/using/integration/sending-audience-data/batch-data-transfer-explained/inbound-ftp-filenames.md) | 我们更新了定义，并为文件 `DPID` 名语 `DPID_TARGET_DATA_OWNER` 法中的字段和字段提供了示例。 |

## August, 2020 {#aam-august-updates}

| 主题 | 描述 |
|---- |----|
| [针对基于人的目标的Google客户匹配](../features/destinations/people-based-destinations-prerequisites.md#gcm) | 您现在可以通 [!UICONTROL People-Based Destinations] 过目标用户 [!DNL Google Customer Match]。 在使用将第 [!UICONTROL People-Based Destinations] 一方受众段发送到目标之前，必 [!DNL Google Customer Match] 须先将您添加到 [!DNL Google] 其允许列表中。 有关详细 [信息，请查看基于人](../features/destinations/people-based-destinations-prerequisites.md#gcm) 员的目标文档。 |
| [新的SFTP服务器](../faq/faq-data-collection.md) | 您现在可以使用 [!DNL SFTP] 新的服[!DNL ftp-in-gtw] 务器( [!DNL ftp-out-gtw]和)，这 [!DNL AWS SFTP Gateway] 些服务 [!DNL S3]器 |
...
| 主题 | 描述 |
|---- |----|
| [在 Audience Manager 与 Adobe Experience Platform 之间共享受众](/help/using/integration/integration-aep/aam-aep-audience-sharing.md#aep-aam-segment-population-differences) | 新的部分“了 **解Audience Manager和Experience Platform之间的细分人口差异**”描述了人口数在Experience Platform和Audience Manager之间可能不同的原因。 |
| [Audience ManagerREST API入门](/help/using/api/rest-api-main/aam-api-getting-started.md#jwt) | 我们使用添加了新的 **部分JWT（服务帐户）身份验证[!DNL Adobe I/O]**。 请参阅本节，使用验证Audience ManagerAPI [!DNL Adobe I/O]。 |
