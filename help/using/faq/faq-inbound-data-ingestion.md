...
能。只要您使用[!UICONTROL cross-device data source]存储您上传的CRM数据，Audience Manager始终存储这些数据。 事实上，在Audience Manager于2019年10月启动的[!UICONTROL Profile Merge Rules]增强功能允许仅离线用例后，您无需将Audience Manager代码部署到生产环境中即可上传数据并对其执行操作。 请参阅：
...
* [!UICONTROL People-based Destinations] - [基于仅脱机数据的个性化](https://experienceleague.adobe.com/docs/audience-manager/user-guide/features/destinations/people-based/implementation-guide/people-based-destinations-workflow-offline.html?lang=zh-Hans)
...
* [Amazon S3入站数据的名称和文件大小要求……](../integration/sending-audience-data/batch-data-transfer-explained/inbound-s3-filenames.md)
...
**什么是[!DNL Data Provider ID (DPID)]？如何获取？**
...
是的。[!DNL Audience Manager] 不会在其数据库中存储电子邮件地址。在启动ID同步之前，应为访问者分配一个随机生成的ID或电子邮件地址的单向散列版本。
...
>[!WARNING]
>
>我们正在逐步停止对 FTP 配置的支持。虽然现有FTP集成仍支持入站数据文件引入，但我们强烈建议使用[!DNL Amazon S3]来加入脱机数据以实现新集成。 有关详细信息，请参阅[入站数据文件的 Amazon S3 名称和文件大小要求](/help/using/integration/sending-audience-data/batch-data-transfer-explained/inbound-s3-filenames.md)。
