...
Adobe为Audience Manager客户提供了[!DNL Amazon S3]存储段的访问密钥。 出于安全考虑，如果这些访问密钥在40天内未使用，则会自动禁用它们。

如果您的访问密钥被禁用，您可以联系客户支持以重新启用它们或请求新的访问密钥。

为了增强安全性，将永久删除180天未使用的访问密钥以及Amazon S3 IAM用户帐户。 如果您是旧客户，并且在此时间段后需要访问，请联系客户支持以重新创建您的帐户并接收新的访问密钥。
