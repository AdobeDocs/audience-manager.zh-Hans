...
+ 功能{#features}
   ...
   + 管理{#administration}
      ...
      + [IP地址模糊处理](features/administration/ip-obfuscation.md)
   ...
   + 算法模型{#algorithmic-models}
      ...
      + Predictive Audiences {#predictive-audiences}
         + [Predictive Audiences概述](features/algorithmic-models/predictive-audiences.md)
         + [Predictive Audiences快速入门](features/algorithmic-models/predictive-audiences-start.md)
         + [Predictive Audiences报表](features/algorithmic-models/predictive-audiences-reporting.md)
   ...
   + 目标{#destinations}
      ...
      + 基于设备的目标（服务器到服务器） {#device-based}
         ...
         + [Audience Manager基于设备的目标列表](/help/using/features/destinations/device-based-destinations-list.md)
         ...
         + [将Amazon Advertising配置为基于设备的自助服务目标](features/destinations/amazon-advertising.md)
   ...
   + 配置文件合并规则{#profile-merge-rules}
      ...
      + [配置文件链接设备图用例](features/profile-merge-rules/profile-link-use-case.md)
   ...
   + 区段{#segments}
      ...
      + [正在检索区段元数据](features/segments/retrieve-segment-metadata.md)
      ...
      + [特征和区段资格参考](https://experienceleague.adobe.com/docs/audience-manager/user-guide/features/traits/trait-and-segment-qualification-reference.html?lang=zh-Hans)
   + 特征{#traits}
      ...
      + [带有平台级别密钥的Geotargeting](features/traits/trait-geotarget-keys.md)
   ...
   + [访客资料查看器](features/visitor-profile-viewer.md)
+ 报表{#reporting}
   ...
   + Audience Optimization报表 {#audience-optimization-reports}
      + [Audience Optimization报表](reporting/audience-optimization-reports/audience-optimization-reports.md)
      + 适用于广告商的Audience Optimization {#audience-optimization-advertisers}
      ...
      + 适用于出版商的Audience Optimization {#audience-optimization-publishers}
      ...
      + Audience Optimization报表的数据和元数据文件以及可操作的日志文件 {#data-and-metadata-files}
         + [日志数据和元数据文件](reporting/audience-optimization-reports/metadata-files-intro/metadata-files-intro.md)
         + [Audience Optimization报表的数据文件以及可操作的日志文件](reporting/audience-optimization-reports/metadata-files-intro/datafiles-intro.md)
         ...
         + [元数据文件的提交方法](reporting/audience-optimization-reports/metadata-files-intro/metadata-delivery-methods.md)
   ...
   + 交互式报表和重叠报表 {#interactive-and-overlap-reports}
      ...
      + [重叠报表的CSV文件](reporting/dynamic-reports/overlap-csv-files.md)
...
+ API和SDK代码{#api-and-sdk-code}
   ...
   + 数据收集服务器(DCS) API方法和代码{#dcs}
      ...
      + DCS API{#dcs-apis}
         ...
         + [从DCS响应中获取用户ID和区域](api/dcs-intro/dcs-s2s/dcs-aam-ids.md)
         + [通过Adobe Experience Platform Identity服务获取用户ID和区域](api/dcs-intro/dcs-s2s/dcs-mcid-ids.md)
      ...
      + DCS API参考{#dcs-api-reference}
         ...
         + [DCS错误代码、消息和示例](api/dcs-intro/dcs-api-reference/dcs-error-codes.md)
...
+ 实施和集成指南{#implementation-integration-guides}
   ...
   + 接收受众数据{#receiving-audience-data}
      ...
      + 实时出站数据传输{#real-time-outbound-transfers}
         ...
         + [经过数字签名的HTTP请求](integration/receiving-audience-data/real-time-outbound-transfers/digitally-signed-http-requests.md)
         + [用于实时出站传输的OAuth 2.0集成](integration/receiving-audience-data/real-time-outbound-transfers/oauth-in-outbound-transfers.md)
      + 批量出站数据传输{#batch-outbound-data-transfers}
         ...
         + [利用您出站文件的Amazon S3跨帐户存储段权限](integration/receiving-audience-data/batch-outbound-transfers/authorize-s3-cross-bucket.md)
...
+ 参考{#reference}
   ...
   + 批量管理工具{#bulk-management-tools}
      ...
      + [批量管理工具的疑难解答提示](reference/bulk-management-tools/bulk-troubleshooting.md)
   ...
   + [了解Demdex域调用](reference/demdex-calls.md)
