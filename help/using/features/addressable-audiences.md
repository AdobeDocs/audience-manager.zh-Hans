...
| 量度 | 描述 |
|---|---|
| [!UICONTROL Audience Manager Addressable Audience]的[!UICONTROL Destination] | 在报表回顾期间在平台级别与所有[!DNL Audience Manager]客户交互并且可能与您选择的[!UICONTROL destination]匹配的所有设备的计数。 <br><br>此量度非常有用，因为它向您显示： <ul><li>[!UICONTROL addressable audience]在特定目标[!DNL Audience Manager]上可达到的总[!UICONTROL destination]的大小。</li><li>[!DNL Audience Manager]配置文件池对于定位平台的大小及其受众的大小。</li></ul> |
...
| [!UICONTROL Customer Addressable Audience] | 在回顾时间范围内实现[!UICONTROL rule-based trait]或[!UICONTROL onboarded trait]的设备以及我们与所选[!UICONTROL destination]具有ID同步的设备的重叠计数，不考虑同步时间。<br><br>此量度表示设备：<ul><li>已在回顾时间范围[!UICONTROL rule-based]中实现[!UICONTROL onboarded trait]或`AND`</li><li>具有与所选[!UICONTROL destination]同步的ID，而不考虑同步时间。</li> |
...
| [!UICONTROL Segment Addressable Audience] | 在报表回顾期间属于[!UICONTROL segment]且在您的网站上具有活动ID同步的用户数。 [!UICONTROL Segments]可以通过在[!UICONTROL traits]Audience Marketplace[中获得的](../features/audience-marketplace/marketplace-data-buyers/marketplace-data-buyers.md)包含您自己的第一方数据以及第二方和第三方数据。 <br><br>提示：与1天回顾期一起使用时，此量度可以帮助您了解[!UICONTROL segments]的当前状态。 这是因为[!UICONTROL Segment Addressable Audience]量度表示前一天在[!UICONTROL segment]中停留的用户。 结合以下事实：[!DNL Audience Manager]每天刷新[!UICONTROL Addressable Audiences]，结合此量度和回顾期间可提供您[!UICONTROL segments]的最新快照。 |
...
| **[!UICONTROL Match Rate]** | 此量度表示过去30天[!UICONTROL Addressable Audience Match Rate]的&#x200B;*（如上表所述）*。 |
...
尝试连接[!DNL Safari]或没有第三方[!DNL cookies]的移动应用用户时存在间隙。 这样将很难将用户与某些合作伙伴同步，因为媒体投放日志中仅提供了已同步第三方[!DNL Adobe]的[!DNL cookies] ID。 这就是为什么您可能会看到[的](../features/addressable-audiences.md#low-match-rates)低匹配率[!UICONTROL destinations]的原因。
...
所有日期和日期范围都设定在[!DNL UTC]时区内。 查看Audience Manager中的[时区](../reference/aam-time-zones.md)。
...
* *不会包含在*&#x200B;选项之前的较短间隔内（当前、7天和14天）[!UICONTROL Last 30 Day]。
...
| 量度 | 描述 |
|---|---|
| [!UICONTROL Customer Addressable Audience] | 在回顾时间范围内实现[!UICONTROL rule-based trait]或[!UICONTROL onboarded trait]的设备以及我们与所选目标具有ID同步的设备的重叠计数，不考虑同步时间。<br><br>此量度表示设备：<ul><li>已在回顾时间范围[!UICONTROL rule-based]中实现[!UICONTROL onboarded trait]或`AND`</li><li>具有与所选[!UICONTROL destination]同步的ID，而不考虑同步时间。</li></ul> |
| [!UICONTROL Customer Total Audience] | 在回顾时间范围内，已在您的属性上实现[!UICONTROL rule-based trait]或从脱机文件中实现[!UICONTROL onboarded trait]的设备计数。 |
| [!UICONTROL Customer Match Rate] | [!UICONTROL Customer Addressable Audience]÷[!UICONTROL Customer Total Audience]以百分比表示。 |
...
| 量度 | 描述 |
|---|---|
| [!UICONTROL Segment Addressable Audience] | 在报表回顾期间属于[!UICONTROL segment]且在您的网站上具有活动ID同步的用户数。 通过在[!UICONTROL traits]Audience Marketplace[中获取的](../features/audience-marketplace/marketplace-data-buyers/marketplace-data-buyers.md)，区段可以包括您自己的第一方数据以及第二方数据和第三方数据。<br><br>提示：与1天回顾期一起使用时，此量度可以帮助您了解[!UICONTROL segments]的当前状态。 这是因为[!UICONTROL Segment Addressable Audience]量度表示前一天在[!UICONTROL segment]中停留的用户。 结合以下事实：[!DNL Audience Manager]每天刷新[!UICONTROL Addressable Audiences]，结合此量度和回顾期间可提供您[!UICONTROL segments]的最新快照。 |
| [!UICONTROL Total Segment Population] | 报表回顾期间属于[!UICONTROL segment]成员的所有设备的计数。 |
| [!UICONTROL Segment Match Rate] | [!UICONTROL Segment Addressable Audience]÷[!UICONTROL Total Segment Population]以百分比表示。 |
...
| 量度 | 描述 |
|---|---|
| [!DNL Audience Manager] [!UICONTROL Addressable Audience] | 在报表回顾期间在平台级别与所有[!DNL Audience Manager]客户交互并且可能与您选择的[!UICONTROL destination]匹配的所有设备的计数。 <br><br>此量度非常有用，因为它向您显示：<ul><li>[!UICONTROL total addressable audience]可以到达特定目标位置的[!DNL Audience Manager]的大小。</li><li>[!DNL Audience Manager]配置文件池对于定位平台的大小及其受众的大小。</li></ul> |
...
* [!UICONTROL Customer Addressable Audiences]基于您自己的第一方数据[!UICONTROL trait]的&#x200B;*实现*。 此量度可让您广泛、全面地了解您与数据合作伙伴的集成。
...
例如，假设您向[!UICONTROL destination]发送了一个区段，但[!UICONTROL destination]显示的报表数量较低。 检查[!UICONTROL Addressable Audience]结果将显示这是技术问题，还是匹配率较低。 低匹配率表明您的[!UICONTROL destination]对于您选择的区段并不是那么好。 但是，[!UICONTROL total addressable audience]和[!DNL Audience Manager]之间的[!UICONTROL destination]数字的差异表示存在集成、同步或其他技术问题。 在这些情况下，请联系您的客户经理。
