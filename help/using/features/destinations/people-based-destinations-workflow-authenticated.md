...
本页包含如何将离线[!DNL CRM]数据与实时行为数据相结合的分步指导，供经过身份验证的用户创建受众段，然后将这些受众段发送到[!DNL People-Based Destinations]。
...
## 第2步——使用声明的ID通过实时HTTP调用{#match-email-addresses}将DPUUID与散列电子邮件地址匹配

要使经过身份验证的用户有资格获得基于规则的特征，您需要通过[声明的ID](../declared-ids.md)发送特征资格。
...
>[!IMPORTANT]
>
>如果已经使用&#x200B;**[!UICONTROL Current Authenticated Profiles]**&#x200B;或&#x200B;**[!UICONTROL Last Authenticated Profiles]**&#x200B;选项定义了规则，则可跳至[步骤4 —— 创建受众段](#create-audience-segments)。
...
5. 在&#x200B;**[!UICONTROL Cross-Device Profile Options]**列表中，选择要运行分段的数据源。 这些数据源应包含现有DPUUID。
...
1. 在&#x200B;**[!UICONTROL Segment Mappings]**&#x200B;部分，选择要发送到此目标的区段。 这将是您在[步骤4 —— 创建受众段](#create-audience-segments)创建的段。
1. 保存目标。
