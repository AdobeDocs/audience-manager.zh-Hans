...
>[!IMPORTANT]
>
> 在继续之前，请确保您即将登入的客户活动已在具有相应已载入特征的Audience Manager [中定义](../traits/trait-and-segment-qualification-reference.md)。

无论您的现有Audience Manager客户ID([DPUUID](../../reference/ids-in-aam.md))是否是散列电子邮件，您都必须对包含DPUUID的数据源执行特 [征注册](../../reference/ids-in-aam.md)。

### 示例

您需要从下表中为相应的已载入特征ID确定客户ID。 让我们考虑一下，您 [的DPUUID](../../reference/ids-in-aam.md) 存储在ID为99999的数据源中，而您的Audience Manager合作伙伴ID为123。
