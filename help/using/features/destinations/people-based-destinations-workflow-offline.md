---
description: '本页包含有关如何根据仅线下受众数据构建客户细分以及将其发送到基于人员的目标的分步指导。  '
seo-description: '本页包含有关如何根据仅线下受众数据构建客户细分以及将其发送到基于人员的目标的分步指导。  '
...
本页包含有关如何根据仅线下受众数据构建客户细分以及将其发送到基于人员的目标的分步指导。

## 步骤1 — 板载脱机特征{#step-1-onboard-traits}

在此方案中，创建受众区段的第一步是将线下客户数据引入Audience Manager。

>[!IMPORTANT]
>
> 在继续之前，请确保您即将登入的客户活动已在具有相应[已载入特征](../traits/trait-and-segment-qualification-reference.md)的Audience Manager中定义。

无论您现有的Audience Manager客户ID([DPUUID](../../reference/ids-in-aam.md))是否是散列电子邮件，您都必须对包含[DPUUID](../../reference/ids-in-aam.md)的数据源执行特性登记。

### 示例

您需要从下表中为相应的已载入特征ID确定客户ID。 让我们考虑一下，您的[DPUUID](../../reference/ids-in-aam.md)存储在ID为999999的数据源中，而您的Audience Manager合作伙伴ID为123。
...
要确定上例中的客户ID对应已载入的特征的资格，您必须上传包含以下内容的[入站数据文件](../../integration/sending-audience-data/batch-data-transfer-explained/inbound-file-contents.md):
...
有关文件名结构的详细信息，请参见[Amazon S3 Name and File Size Requirements for Inbound Data Files](../../integration/sending-audience-data/batch-data-transfer-explained/inbound-s3-filenames.md)。
...
## 步骤3 — 通过基于文件的ID同步{#match-ids-emails}将DPUUID与散列电子邮件地址匹配

>[!IMPORTANT]
>
> 此步骤仅适用于上述[方案2](people-based-destinations-workflow-offline.md#configure-data-source-settings)。 如果您现有的[DPUUID](../../reference/ids-in-aam.md)已经具有散列的电子邮件地址，请跳到[步骤4 — 为分段创建用户档案合并规则](#create-profile-merge-rule)。

假设您要将第1步示例中的现有[DPUUID](../../reference/ids-in-aam.md)与下表（右列）中的哈希电子邮件地址匹配，并将哈希电子邮件地址存储在您在[第2步 — 配置数据源设置](#configure-data-source-settings)创建的新数据源中。
...
5. 在&#x200B;**[!UICONTROL Cross-Device Profile Options]**列表中，选择要载入您的特征的数据源。
   ![合并规则设置](assets/pbd-pmr.png)

## 步骤5 — 创建受众区段{#create-audience-segments}

要从仅脱机用户档案创建新区段，请使用[区段生成器](../segments/segment-builder.md)，并确保在创建区段时使用在上一步创建的新合并规则。
...
1. 在&#x200B;**[!UICONTROL Segment Mappings]**&#x200B;部分，选择要发送到此目标的区段。 这将是您在[步骤5 — 创建受众区段](people-based-destinations-workflow-offline.md#create-audience-segments)创建的区段。
1. 保存目标。
