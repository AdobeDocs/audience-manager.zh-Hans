---
description: 合并规则选项允许您控制Audience Manager用于分段的数据类型。 合并规则可以包括由用户档案链接设备图形、Adobe Experience Cloud设备合作社和／或其他与Audience Manager集成的第三方设备图形提供商映射的设备用户档案。 最多可创建4个用户档案合并规则。
seo-description: 合并规则选项允许您控制Audience Manager用于分段的数据类型。 合并规则可以包括由用户档案链接设备图形、Adobe Experience Cloud设备合作社和／或其他与Audience Manager集成的第三方设备图形提供商映射的设备用户档案。 最多可创建4个用户档案合并规则。
...
使用[!UICONTROL profile merge rule]选项可以控制[!DNL Audience Manager]用于分段的数据类型。 [!UICONTROL profile merge rule]可以包括由[!UICONTROL Profile Link]设备图形、[!UICONTROL Adobe Experience Cloud Device Co-op]和／或与[!DNL Audience Manager]集成的其他第三方设备图形提供者映射的设备用户档案。 最多可创建4个[!UICONTROL Profile Merge Rules]。 第四个[!UICONTROL Profile Merge Rule]只适用于购买[!UICONTROL People-Based Destinations]加载项的客户。

通过在[!UICONTROL Profile Merge Rule Setup]中从下面描述的选项中进行选择，可生成[!UICONTROL Profile Merge Rule]。
...
| [!UICONTROL All Cross-Device Profiles] | 不适用 | 对[基于人员的目标](../destinations/people-based-destinations-overview.md)客户独有 | 仅批 | 否 | [针对基于人的目标进行定位](merge-rule-targeting-options.md#all-cross-device) |
...
* 对先前限定的[!UICONTROL traits]执行批处理[!UICONTROL segment]评估。
* [!UICONTROL Profile Merge Rules] 支持实时和批量评估 [!UICONTROL segment] 的访客活动与先前的合格相结合 [!UICONTROL traits]。

## [!UICONTROL Profile Merge Rules] 报告延迟  {#reporting-latency}

实时[!UICONTROL segment]评估会立即反映在[!UICONTROL Profile Merge Rules]报告中。

批[!UICONTROL segment]评估最长可能需要24小时，才能反映在[用户档案合并规则报告](profile-link-metrics.md)中。

## [!UICONTROL Cross-Device Options] {#auth-options}

[!UICONTROL Cross-Device Options]允许您选择已验证和未验证的用户，并利用其跨设备用户档案进行细分。 这些选项可帮助您识别并触及共享设备上的特定用户。 有关匿名和已验证用户的详细信息，请参阅Audience Manager](../../reference/visitor-authentication-states.md)中的[访客身份验证状态。
...
<table id="table_4CE2DD312F54480E96BEAF72800789FB"> 
 ...
 <tbody> 
  <tr> 
   ...
   <td colname="col2"> <p>通知<span class="keyword">Audience Manager</span>不要使用从已验证用户收集的数据。 </p> </td> 
  </tr> 
  <tr> 
   <td colname="col1"> <p> <b><span class="uicontrol"> 当前已验证的用户档案</span></b> </p> </td> 
   <td colname="col2"> <p>告知<span class="keyword">Audience Manager</span>在访客已登录您的站点时，将数据读写到已验证的用户档案。 </p> </td> 
  </tr> 
  <tr> 
   <td colname="col1"> <p> <b><span class="uicontrol"> 上次验证用户档案</span></b> </p> </td> 
   <td colname="col2"> <p>通知<span class="keyword">Audience Manager</span>从上次登录设备的用户的已验证用户档案读取数据。 </p> <p>选择<span class="keyword">Audience Manager</span>后，如果用户为匿名用户档案，则该用户将不会向已验证的数据写入新特征数据。 验证后，新特征数据将写入用户的已验证用户档案。 </p> </td>
  </tr> 
  <tr> 
...
<table id="table_D373FB787D1A4E3485C02C4A76F03395"> 
 ...
 <tbody> 
  <tr> 
   ...
   <td colname="col2"> <p>通知<span class="keyword">Audience Manager</span>不要使用匿名用户档案中包含的特征进行分段。 </p> </td> 
  </tr> 
  <tr> 
   <td colname="col1"> <p> <b><span class="uicontrol"> 设备用户档案</span></b> </p> </td> 
   <td colname="col2"> <p>告知<span class="keyword">Audience Manager</span>使用匿名设备用户档案进行分段。 </p> </td> 
  </tr> 
  <tr> 
   <td colname="col1"> <p> <b><span class="uicontrol"> 用户档案链接设备图</span></b> </p> </td> 
   <td colname="col2"> <p>通知<span class="keyword">Audience Manager</span>从当前设备和最多100个用户已通过身份验证的其他设备读取用户档案。 此设备图形基于您自己的第一方Audience Manager<span class="keyword"></span>构建。 它非常适合那些在数字资产中具有高级身份验证的客户。 实时更新<span class="wintitle">用户档案链路</span>设备图形。 当您选择<b><span class="uicontrol">当前已验证用户档案</span></b>或<b><span class="uicontrol">上次已验证用户档案</span></b>时，此选项可用。 使用此选项时，您只能选择单个已验证的用户档案(<span class="keyword">Audience Manager</span>会自动灰化其他数据)。 另请参阅<a href="profile-link-use-case.md">用户档案链路设备图表用例</a>。 </p> </td>
  </tr> 
  <tr> 
   <td colname="col1"> <p> <b><span class="uicontrol"> 协作设备图</span></b> </p> </td> 
   <td colname="col2"> <p>指示<span class="keyword">Audience Manager</span>使用<a href="https://docs.adobe.com/content/help/en/device-co-op/using/about/overview.html" format="https" scope="external">Experience Cloud设备Co-op</a>提供的链接从当前设备和最多100个其他设备读取用户档案。 </p> <p><span class="keyword"> 设备协作</span>是一项数字协作服务，可让参与的客户共享设备链接信息。<span class="keyword">设备协作</span>在<span class="term">设备图形</span>中处理此数据。 设备图形将设备连接在一起，形成设备群集。 这些链接是从<a href="https://docs.adobe.com/content/help/en/device-co-op/using/device-graph/links.html" format="https" scope="external">概率数据和确定性数据</a>构建的。 这些群集代表由未知人使用的一组设备。 <span class="keyword">设备协作</span>会在其成员之间共享这些群集，从而帮助成员向其客户提供富有价值的、一致的跨设备体验。 </p> <p> 有关<span class="wintitle">设备合作社</span>的详细信息，请参见： </p> <p> 
  ...
  <tr> 
   <td colname="col1"> <p><b>第三方设备图选项</b> （人和家） </p> </td>
   <td colname="col2"> <p>这些选项允许您根据第三方供应商提供的设备图技术构建合并规则。 第三方设备图表提供： </p> <p> 
     ...
     </ul> </p> <p>要使用这些选项，您必须是设备图形的客户，设备图形提供者已与<span class="keyword">Audience Manager</span>集成。 请联系您的客户经理以了解更多信息或开始。 </p> </td>
  </tr>
 </tbody>
</table>
...
根据在[!DNL Audience Manager]之外定义的合并规则，从其他[!DNL Experience Cloud]解决方案自动创建的受众段被标记为使用[!UICONTROL External Merge Policy]。 例如，请参阅[受众与Adobe Experience Platform之间共享](../../integration/integration-aep/aam-aep-audience-sharing.md)。
