...
The merge rule options let you control the type of data [!DNL Audience Manager] uses for segmentation. 合并规则可以包括由设备图形、 [!UICONTROL Profile Link] 和／或与 [!UICONTROL Adobe Experience Cloud Device Co-op]集成的其他第三方设备图形提供者映射的设备用户档案 [!DNL Audience Manager]。 最多可创建4个 [!UICONTROL Profile Merge Rules]。 第四 [!UICONTROL Profile Merge Rule] 项仅适用于购买该加载项 [!UICONTROL People-Based Destinations] 的客户。
...
根据您 [!UICONTROL Profile Merge Rules] 的配 [!DNL Audience Manager] 置，可以实时、批量或同时执行区段评估。
...
根据外部定义的合并规则，从其他受众解决方案自动创建的Experience Cloud [!DNL Audience Manager]区段会被标记为使用 [!UICONTROL External Merge Policy]。 例如，请参阅 [受众在Audience Manager和Adobe Experience Platform之间共享](../../integration/integration-aep/aam-aep-audience-sharing.md)。
