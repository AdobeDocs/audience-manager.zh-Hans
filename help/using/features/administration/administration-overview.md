---
description: “管理”菜单下的选项允许您创建Audience Manager用户并将其分配给组。 您还可以查看限制（特征、区段、目标和模型）。
...
>[!IMPORTANT]
>
> 在使用[!DNL RBAC]之前，必须由Adobe为您的组织启用此功能。 请联系您的帐户团队以请求[!DNL RBAC]激活，或联系客户关怀团队。
...
您可以为每个用户组在[!UICONTROL trait]、区段和[!UICONTROL destination]级别配置[!UICONTROL Role-Based Access Control]。
