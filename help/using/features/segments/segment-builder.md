...
1. 在[基本信息](../../features/segments/segment-builder.md#segment-builder-controls-basics)部分中：
   ...
   * 选择[!UICONTROL data source]。 使用第一个下拉菜单在Audience Manager[!UICONTROL data sources]、Adobe Analytics报表包或两者之间筛选。 然后，使用第二个下拉菜单选择您的[!UICONTROL data source]。 如果您未使用Adobe Analytics报表包，则[!UICONTROL data source]类型选择器将处于禁用状态，并且默认仅Audience Manager数据源。
   * 选择要用于[!UICONTROL segment]资格的[!UICONTROL profile merge rule]。
...
1. 在[特征](../../features/segments/segment-builder.md#segment-builder-controls-traits)部分中：
   ...
   * 通过单击&#x200B;**[!UICONTROL Browse All Traits]**&#x200B;打开[!UICONTROL Advanced Search]模式窗口。 按名称、ID、说明或[!UICONTROL data source]搜索[!UICONTROL traits]。 搜索时单击某个文件夹，将结果限制为该文件夹及其子文件夹。 您还可以按[!UICONTROL trait type] （[!UICONTROL Folder Trait]、[!UICONTROL Rule-based]、[!UICONTROL Onboarded]和[!UICONTROL Algorithmic]）或填充类型（[设备ID](../../reference/ids-in-aam.md)和[跨设备ID](../../reference/ids-in-aam.md)）筛选[!UICONTROL traits]。
     ![segment-builder-browser-traits](assets/segment-builder-browse-traits.png)
   * 在构建[!UICONTROL segment]时实时获取[特征推荐](trait-recommendations.md)。
   ...
   * 在添加或删除[!UICONTROL traits]时查看区段填充数据。 单击&#x200B;**[!UICONTROL Calculate Estimates]**&#x200B;查看（或刷新）预计的群体数量。 详细了解[!UICONTROL Segment Builder]中的[区段人口数据](../../features/segments/segment-builder-data.md#segment-populations)。
   * 完成后单击&#x200B;**[!UICONTROL Save]**。

1. *（可选）*&#x200B;将[!UICONTROL segment]映射到[目标映射](../../features/segments/segment-builder.md#segment-builder-controls-destinations)部分中的[!UICONTROL destination]：
...
| **[!UICONTROL Data Source]** | 将区段与特定数据提供商关联。 <br>使用第一个下拉菜单在Audience Manager数据源、Adobe Analytics报表包或两者之间筛选。 然后，使用第二个下拉菜单选择数据源。 <br>如果您未使用Adobe Analytics报表包，则数据源类型选择器将处于禁用状态，并且默认为仅Audience Manager数据源。 |
...
在[!UICONTROL Segment Builder]中，[!UICONTROL Traits]部分允许您在[!UICONTROL segment]中管理[!UICONTROL traits]、创建[!UICONTROL trait]组并设置资格条件。 要将[!UICONTROL trait]添加到[!UICONTROL segment]，请在搜索字段中键入[!UICONTROL trait]名称，然后单击[!UICONTROL Add Trait]。 保存[!UICONTROL trait]（如果完成）或移至[!UICONTROL Destinations Mapping]。
...
| 字段 | 描述 |
|--- |--- |
| **[!UICONTROL Basic View]** | 此部分提供了可视控件，这些控件允许您： <ul><li>生成新的和管理现有的[!UICONTROL segments]。</li><li>从[!UICONTROL segment]中删除[!UICONTROL traits]。</li><li>将最多50个（最大） [!UICONTROL traits]添加到[!UICONTROL segment]。</li><li>拖放[!UICONTROL traits]以创建新组。</li><li>在[!UICONTROL segment]中查看[!UICONTROL traits]和[!UICONTROL trait]组。</li><li>使用布尔表达式、比较运算符和回访间隔/频率设置设置资格标准。</li></ul> |
| **[!UICONTROL Code View]** | 打开一个开发环境，允许您使用代码而不是可视化界面创建和管理[!UICONTROL traits]、组和资格要求。 如果您的[!UICONTROL segments]： <ul><li>单个[!UICONTROL segment]中的[!UICONTROL traits]超过50个。 注意： [!UICONTROL Segments]限制为5000 [!UICONTROL traits]（最大值）。</li><li>包含多个[!UICONTROL trait]组。</li><li>具有复杂的资格要求。</li></ul> |
| 搜索 | 帮助您找到要添加到[!UICONTROL segment]的[!UICONTROL traits]。 |
| 推荐 | 从您订阅的第一方[!UICONTROL traits]和[!UICONTROL Audience Marketplace]数据馈送中获取类似[!UICONTROL traits]的实时推荐。 将这些推荐添加到[!UICONTROL segment]规则以扩展您的受众。 请参阅[特征Recommendations](trait-recommendations.md)以了解详情。 |
| **[!UICONTROL Marketplace Recommendations]** | 从您未订阅的[!UICONTROL Audience Marketplace]数据馈送获取类似[!UICONTROL traits]的实时推荐。 请参阅[特征Recommendations](trait-recommendations.md)以了解详情。 |
| 实际和预计[!UICONTROL Segment]大小数据 | 请参阅[区段生成器中的特征和区段人口数据](segment-builder-data.md)。 |

## 从[!UICONTROL Segment]中删除[!UICONTROL Traits] {#remove-traits}

管理[!UICONTROL segments]中的[!UICONTROL traits]是保持[!UICONTROL segments]可行性的重要部分。 如果需要从[!UICONTROL segment]中移除[!UICONTROL traits]，请按照以下步骤操作。
...
4. 将鼠标悬停在要删除的[!UICONTROL trait]上，然后单击X。此操作会立即从您的[!UICONTROL segment]中删除[!UICONTROL trait]。
...
在[!UICONTROL Segment Builder]中，选择[!UICONTROL destination]后将显示[!UICONTROL Add Destination]对话框。 此窗口显示有关[!UICONTROL destination]的静态信息以及根据[!UICONTROL destination]类型而变化的字段。 在空字段中提供设置[!UICONTROL destination mapping]所需的信息。
