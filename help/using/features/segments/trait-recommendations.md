...
由[!UICONTROL Trait Recommendations]提供支持的[!DNL Adobe Sensei]将数据科学融入您的Audience Manager日常工作流程。
使用[!UICONTROL Trait Recommendations]，在[区段生成器](segment-builder.md)中生成或编辑区段时，您能够获得可包含的其他推荐特征（与区段规则中的特征类似）。

Audience Manager在&#x200B;**[!UICONTROL Recommendations]**&#x200B;部分和&#x200B;**[!UICONTROL Audience Marketplace]**&#x200B;部分分别显示来自第一方特征和&#x200B;**[!UICONTROL Recommendations from Marketplace]**&#x200B;的特征推荐。
...
* Audience Manager在[!UICONTROL Recommendations]部分中显示第一方特征。 来自您未订阅的公开和私有信息源的Marketplace建议将显示在[!UICONTROL Recommendations from Marketplace]部分中。 单击馈送名称以转到[!UICONTROL Audience Marketplace]并订阅。
* Audience Manager最多可显示50个与区段规则中特征相似的特征。
* 您可以过滤掉不想查看任何推荐的数据源。
* 计算相似性时，Audience Manager会考虑过去30天内符合特征条件的[个UUID](../../reference/ids-in-aam.md)。
* 如果您看到错误消息“未找到类似的特征。 特征可能太新。”，这表示该特征在过去30天内没有活动，或者Audience Manager尚未更新针对该特征的推荐。 请在24小时后重试。
...
[!UICONTROL Algorithmic Models]不仅会找到最具影响力的特征，还会根据这些特征对用户进行评分，并为每个用户分配一个得分。 然后，便可以通过创建算法特征来锁定用户。利用[!UICONTROL Trait Builder]中的精度和范围控件，您可以指定哪些用户具有要定位的有影响力特征。
...
1. 您可以在[!UICONTROL Audience Marketplace]部分中查看来自您订阅的馈送的第一方推荐特征和&#x200B;**[!UICONTROL Recommendations]**&#x200B;特征推荐。 **[!UICONTROL Recommendations from Marketplace]**&#x200B;部分显示来自您未订阅的源的特征推荐。 所有这些推荐都与您添加到区段规则的特征类似。 向下滚动查看所有推荐的特征。
...
为了生成特征推荐，Audience Manager将计算目标特征与您帐户有权访问的所有其他特征（包括第三方数据）之间的[Jaccard相似度](https://en.wikipedia.org/wiki/Jaccard_index)。 然后，Audience Manager会显示最多五十个具有最高相似度的特征。

## 特征相似度分数 {#trait-similarity-score}

Audience Manager通过计算[!UICONTROL Trait Similarity Score]个数的交集和并集来计算两个特征之间的[!UICONTROL UUID]，然后将两者相除。 对于两个特征A和B，计算方式如下所示：
...
在[!UICONTROL RBAC]此处[阅读有关](../administration/administration-overview.md)控件的更多信息。

## 限制

* 目前，Audience Manager不将文件夹特征显示为推荐的特征。 在[此处](../traits/manage-folder-traits.md)阅读有关文件夹特征的更多信息。
* 显示特征推荐时，Audience Manager在区段规则中未考虑[!DNL Boolean]运算符([!DNL AND]、[!DNL OR]、[!DNL NOT])。
