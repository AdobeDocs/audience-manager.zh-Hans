...
# 入站数据文件的[!DNL Amazon S3]名称和文件大小要求 {#amazon-s-name-and-file-size-requirements-for-inbound-data-files}

描述向[!DNL Audience Manager]发送数据时需要遵循的必填字段、语法、命名惯例和文件大小。 在向[!DNL Audience Manager] / [!DNL Amazon S3]目录发送数据时，根据这些规范设置文件的名称和大小。
...
<table id="table_455D174BAB9B494D973DA1023F22B962"> 
 <thead> 
  <tr> 
   <th colname="col1" class="entry"> 为元素命名 </th> 
 ...
 <tbody> 
  <tr> 
   ...
   <td colname="col2"> <p>Amazon S3存储段的路径和名称。 请联系您的客户经理，以获取您的S3目录名称、路径和凭据。 </p> </td> 
  </tr> 
  <tr> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p>将文件发送到Amazon S3时，请仅使用gzip压缩。 压缩后，这些文件将获得<code> .gz</code>扩展名。 请勿使用.zip压缩。 </p> <p>压缩文件必须为3 GB或更小。 如果您的文件较大，请联系客户关怀团队。 虽然Audience Manager可以处理大型文件，但我们或许能够帮助您减小文件大小并提高数据传输效率。 请参阅入站数据传输文件的<a href="../../../integration/sending-audience-data/batch-data-transfer-explained/inbound-file-compression.md">文件压缩</a>。 </p> </td> 
  </tr> 
 </tbody> 
</table>
...
如果您需要其他示例，可以[下载](assets/ftp_dpm_1234_1445374061.overwrite)示例文件。 此文件已使用`.overwrite`文件扩展名保存。 使用简单的文本编辑器将其打开。
...
入站数据文件的行长度限制为102400字节。 超出此限制的行将从传输中排除。
