...
入站数据文件中的字段必须按以下顺序显示。 在此示例中，已添 `<``>` 加符号以帮助以可视方式分隔每个元素。 您无需在数据文件中包含这些组件。

```
<user ID><TAB><trait ID>,<trait ID>,<trait ID>,...
```

有关其他已接受的文件内容格式，请参阅自 [定义合作伙伴集成](/help/using/integration/sending-audience-data/custom-partner-integrations.md)。

>[!NOTE]
>
>我们对入站数据文件中发送的每个用户ID有200行的限制。 例如，如果您为用户ID发送300行，则前200行将保留，另外100行将被丢弃。 在以下示例中，您做得不错，因为您每行发送3行，分别代表用户ID 1和用户ID 2。 我们不对您在行中包含的特征或键值对数量施加限制。
>
>```
...
<table id="table_FE043CE392B34D5194111188E5C39671"> 
 ...
 <tbody> 
  <tr> 
   ...
   <td colname="col2"> <p>用户ID可以是： </p> <p> 
     <ul id="ul_25168355353545A9A049D0083403025E"> 
      <li id="li_23829FE2F6464E33859B3E388FCD106B">由Audience Manager分配的唯一用 <span class="keyword"> 户ID </span> ( <a href="../../../reference/ids-in-aam.md"> Audience Manager UUID </a>)。 </li> 
      <li id="li_76961F20DD3F4554AD2ADFB773F975DB">在CRM系统中分配的唯一用户ID( <a href="../../../reference/ids-in-aam.md"> Audience manager中的DPUUID </a>)。 </li> 
      <li id="li_52ABF6CCBCD147E2BD84D056F7461BA0">移动Android或iOS设备ID，其原始未修改的形式由移动操作系统公开。 </li> 
     </ul> </p> <p>对于移动ID: </p> <p> 
     <ul id="ul_717A17E11565427E9E2D9D7554BB231B"> 
      <li id="li_83BC5EA1E0294651A1F11D7E78EBCE98">IDFA格式：ID必须是大写的，而不是哈希的。 For example, <code> 6D92078A-8246-4BA4-AE5B-76104861E7DC </code> </li> 
      <li id="li_27F298E62A1E46F88ECF52A01B752D3A">Android格式：ID必须小写，而不是散列。 For example, <code> 97987bca-ae59-4c7d-94ba-ee4f19ab8c21 </code> </li> 
     </ul> </p> </td> 
  </tr> 
  <tr> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p>Audience <span class="keyword"> Manager特 </span> 征ID。 我们要求您在入站数 <i>据文件中仅包含已载入</i> 的特征。 在入站数据传输中，我们不会处理任何其他特征类型。 </p> <p> <p>注意： 可以使用GET方法找到特征ID，该方法返回有关您所有特征的详细信息。 有关详细信息，请参阅 <a href="../../../api/rest-api-main/api-traits.md"> 特征API方法 </a>。 </p> </p> </td> 
  </tr> 
 </tbody> 
</table>
...
下表描述了标识入站数据文件中的特征名称或ID的前缀。 有关示例 [，请参阅](../../../integration/sending-audience-data/batch-data-transfer-explained/inbound-file-contents.md#data-file-examples) 示例文件。
...
<table id="table_AD54B3E5487E47C481A4E5FD3A93FDA5"> 
 ...
 <tbody> 
  <tr> 
   ...
   <td colname="col2"> <p>前 <code> d_sid </code> 缀告诉我们的系统该ID是 <span class="keyword"> Audience Manager特 </span> 征ID。 这是用户界面中显示的同一ID。 您还可以使用API方法返回特征 <code> GET </code> ID。 请参 <a href="../../../api/rest-api-main/api-traits.md"> 阅特征API方法 </a>。 </p> </td>
  </tr>
  <tr> 
   <td colname="col1"> <p> <code> d_unsid= </code> </p> </td> 
   <td colname="col2"> <p>前缀为的数 <code> d_unsid </code> 据会从该特征中删除用户。 在文 <code> d_unsid </code> 件中忽略前缀 <code> overwrite </code> 。 </p> <p>前 <code> d_unsid= </code> 缀告诉我们的系统该ID是 <span class="keyword"> Audience Manager特 </span> 征ID。 这是用户界面中显示的同一ID。 您还可以使用API方法返回特征 <code> GET </code> ID。 请参 <a href="../../../api/rest-api-main/api-traits.md"> 阅特征API方法 </a>。 </p> </td>
  </tr> 
  <tr> 
   <td colname="col1"> <p> <code> ic= </code> </p> </td> 
   <td colname="col2"> <p> <a href="../../../features/traits/manage-trait-rules.md#managing-trait-rules"> 特征规则 </a> 允许您设置特征资格标准。 如果将特征规则格式化为 <code> ic == trait ID </code>，则可以在简单的逗号格式列表中发送特征。 </p> <p>例如，假设您创建了以下3个特征规则： </p> <p> 
     <ul class="simplelist"> 
      <li> <code> ic == "123" </code> </li>
      <li> <code> ic == "456" </code> </li>
      <li> <code> ic == "789" </code> </li>
     </ul> </p> <p>这些特征与键相关联 <code> ic </code> 。 这样，您就可以在数据文件中创建更简单的特征列表。 而且，您无需包含前缀 <code> ic </code> 。 因此，数据文件的内容可能如下： </p> <p>
     <code> 
      &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; 
...
特征ID仅由数字字符组成。 我们要求您在入站数 *据文件中仅包含已载入* 的特征。 在入站数据传输中，我们不会处理任何其他特征类型。
...
<table id="table_8C5C7271B813441EA2D45CA2FE2A6C59"> 
 ...
 <tbody> 
  <tr> 
   ...
   <td colname="col2"> <p><i>请勿使用已编码的冒号</i> ()或未编码的冒号 <code> %3A </code>(:)符号。 </p> </td> 
  </tr> 
  <tr> 
   <td colname="col1"> <p>移动iOS(IDFA)或Android设备ID </p> </td> 
   <td colname="col2"> <p>移动设备ID必须严格格式化，如下所示： </p> <p> 
     <ul id="ul_6AEFB6CFA54444D9B75F03BCE7916696"> 
      <li id="li_45B272D5EEE944FC9D5C89A0924465F7">IDFA格式：ID必须是大写的，而不是哈希的。 For example, <code> 6D92078A-8246-4BA4-AE5B-76104861E7DC </code> </li> 
      <li id="li_2DA0347293814C70ADCD253BF01A81F5">Android格式：ID必须小写，而不是散列。 For example, <code> 97987bca-ae59-4c7d-94ba-ee4f19ab8c21 </code> </li> 
     </ul> </p> </td>
  </tr>
 </tbody>
</table>
...
<table id="table_41A4991090A64DEFA9AF704164B26DBB"> 
 ...
 <tbody> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p>我们忽略键头的短划线符号。 例如， <code> -product = camera </code> 解释为 <code> product = camera </code>。 </p> </td> 
  </tr> 
  <tr> 
   <td colname="col1"> <p> <code> TAB </code> </p> </td> 
   <td colname="col2"> <p><i>请勿在键</i> -值 <code> TAB </code> 对中使用空值。 仅用于 <code> TAB </code> 在入站数据文件中分隔变量。 </p> </td> 
  </tr> 
  <tr> 
   <td colname="col1"> <p> <code> \n, \t </code> </p> </td> 
   <td colname="col2"> <p>请勿在键或值中使用新行或制表符( <code> \n, \t </code>)。 </p> </td> 
  </tr>
 </tbody>
</table>
...
<table id="table_8017E070F7A54143A82CA153CBAEB5DA"> 
 ...
 <tbody> 
  <tr> 
   ...
   <td colname="col2"> <p>此数据文件显示符合特征24、26、27条件的用户，并且已从特征28和29中删除。 </p> <p> 
     <code>
       59767559181262060060278870901087098252&amp;nbsp;&amp;nbsp;d_sid=24,d_sid=26,d_sid=27,d_unsid=28,d_unsid=29 
     </code> </p> <p>注释:  <p>除了使用d_unsid，您还可以使用以下语法从用户配置文件中删除特征： </p> <p> 
      <code>
  ...
  <tr> 
   ...
   <td colname="col2"> <p>这些特征已添加到带前缀的特征规 <code> ic </code> 则中。 因此，您可以将它们添加到以逗号分隔的数据文件中，如所示。 选项卡用于分隔UUID和特征ID。 文 <code> ic </code> 件中不需要前缀。 </p> <p><b>数字ID</b> </p> <p> 
     <code>
...
[如果需要](assets/ftp_dpm_1234_1445374061.overwrite) ，请下载示例数据文件。 下载文件具有文 `.overwrite` 件扩展名。 您可以使用简单的文本编辑器打开它。

## 示例矩阵 {#examples-matrix}

下图显示了如何正确设置入站文件格式的示例，具体取决于ID的类 [型](../../../reference/ids-in-aam.md) ，以及向配置文件添加特征的方法。
...
<table id="table_FE6D97A1F5074E4A8EFC723AF0C5E707"> 
 <thead> 
  <tr> 
   ...
   <th colname="col4" class="entry"> 发送键值对以向用户配置文件添加特征 </th> 
 ...
 <tbody> 
  ...
  <tr> 
   ...
   <td colname="col4"> <p> <a href="../../../integration/sending-audience-data/batch-data-transfer-explained/inbound-file-contents.md#example-11"> 示例10 </a> </p> </td> 
...
使用特征ID发送Audience Manager UUID的特征资格信息。
...
使用特征ID发送Audience Manager UUID的特征取消资格信息。
...
发送键值对以添加Audience Manager UUID的特征资格信息。
...
使用ic前缀发送Audience Manager UUID的特征资格信息。
