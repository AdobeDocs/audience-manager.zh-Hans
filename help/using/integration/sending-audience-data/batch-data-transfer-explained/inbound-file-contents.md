...
seo-description: 设置入站特征数据文件格式时，应遵循的必填字段、语法和规则。
...
<table id="table_FE043CE392B34D5194111188E5C39671"> 
 ...
 <tbody> 
  <tr> 
   ...
   <td colname="col2"> <p>用户ID可以是： </p> <p> 
     <ul id="ul_25168355353545A9A049D0083403025E"> 
      ...
      <li id="li_76961F20DD3F4554AD2ADFB773F975DB">在CRM系统中分配的唯一用户ID(<a href="../../../reference/ids-in-aam.md"> DPUUID，在Audience Manager</a>中)。 </li> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p><span class="keyword">Audience Manager</span>特征ID。 我们要求您在入站数据文件中仅包含<i>已载入的特征</i>。 在集客数据传输中，我们不会处理任何其他特征类型。 </p> <p> <p>注意： 特征ID可使用返回所有特征详细信息的GET方法找到。 有关更多信息，请参阅<a href="../../../api/rest-api-main/api-traits.md">特征API方法</a>。 </p> </p> </td> 
  </tr> 
 </tbody> 
</table>
...
下表描述了用于标识入站数据文件中[!UICONTROL trait]名称或ID的前缀。 有关示例，请参阅[示例文件](../../../integration/sending-audience-data/batch-data-transfer-explained/inbound-file-contents.md#data-file-examples)。
...
<table id="table_AD54B3E5487E47C481A4E5FD3A93FDA5"> 
 ...
 <tbody> 
  <tr> 
   ...
   <td colname="col2"> <p><code> d_sid </code>前缀用于告知我们的系统ID是<span class="keyword">Audience Manager</span>特征ID。 此ID与用户界面中显示的ID相同。 您还可以使用API <code> GET </code>方法返回特征ID。 请参阅<a href="../../../api/rest-api-main/api-traits.md">特征API方法</a>。 </p> </td>
  </tr>
  <tr> 
   <td colname="col1"> <p> <code> d_unsid= </code> </p> </td> 
   <td colname="col2"> <p>带有<code> d_unsid </code>前缀的数据会从该特征中删除用户。 在<code> overwrite </code>文件中，将忽略<code> d_unsid </code>前缀。 </p> <p><code> d_unsid= </code>前缀用于告知我们的系统ID是<span class="keyword">Audience Manager</span>特征ID。 此ID与用户界面中显示的ID相同。 您还可以使用API <code> GET </code>方法返回特征ID。 请参阅<a href="../../../api/rest-api-main/api-traits.md">特征API方法</a>。 </p> </td>
  </tr> 
  <tr> 
   <td colname="col1"> <p> <code> ic= </code> </p> </td> 
   <td colname="col2"> <p> <a href="../../../features/traits/manage-trait-rules.md#managing-trait-rules"> 特征规则 </a> 允许您设置特征鉴别标准。如果将特征规则格式设置为<code> ic == trait ID </code>，则可以以简单的逗号分隔列表形式发送特征。 </p> <p>例如，假设您创建了以下3个特征规则： </p> <p> 
     <ul class="simplelist"> 
      <li> <code> ic == "123" </code> </li>
      <li> <code> ic == "456" </code> </li>
      <li> <code> ic == "789" </code> </li>
     </ul> </p> <p>这些特征与<code> ic </code>键相关联。 这可让您在数据文件中创建更简单的特征列表。 而且，您不需要包含<code> ic </code>前缀。 因此，数据文件的内容可能如下所示： </p> <p>
     <code> 
      &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; 
  ...
  <tr> 
   ...
   <td colname="col2"> <p>特征数据可以使用字母数字字符串以键值对的形式进行格式设置。 有多种格式化键值对的方法，如下所示： </p> <p> 
     ...
     </ul><code> "age"="32" </code> 、  <code> "gender"=m </code> 、 <code> model = "pickup truck" </code> 均 <code> product = tablet </code> 是格式正确的键值对的示例。 </p> </td> 
  </tr>
 </tbody>
</table>
...
[!UICONTROL Trait IDs] 只包含数字字符。我们要求您在入站数据文件中仅包含&#x200B;*[!UICONTROL onboarded traits]*。 在集客数据传输中，我们不会处理任何其他[!UICONTROL trait]类型。
...
<table id="table_8C5C7271B813441EA2D45CA2FE2A6C59"> 
 ...
 <tbody> 
  <tr> 
   ...
   <td colname="col2"> <p><i>请</i> 勿使用编码的冒号( <code> %3A </code>)或未编码的冒号(:)符号。 </p> </td> 
  </tr> 
  <tr> 
   <td colname="col1"> <p>移动设备iOS(IDFA)或Android设备ID </p> </td> 
...
<table id="table_41A4991090A64DEFA9AF704164B26DBB"> 
 ...
 <tbody> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p>我们在键的开头忽略掉的符号。 例如， <code> -product = camera </code>被解释为<code> product = camera </code>。 </p> </td> 
  </tr> 
  <tr> 
   <td colname="col1"> <p> <code> TAB </code> </p> </td> 
   <td colname="col2"> <p><i>请</i> 勿 <code> TAB </code> 在键值对中使用空值。仅使用<code> TAB </code>在集客数据文件中分隔变量。 </p> </td> 
  </tr> 
  <tr> 
   <td colname="col1"> <p> <code> \n, \t </code> </p> </td> 
   <td colname="col2"> <p>请勿在键或值中使用新的行或制表符(<code> \n, \t </code>)。 </p> </td> 
  </tr>
 </tbody>
</table>
...
<table id="table_8017E070F7A54143A82CA153CBAEB5DA"> 
 ...
 <tbody> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p>这些特征已添加到具有<code> ic </code>前缀的特征规则中。 因此，您可以将它们添加到以逗号分隔的数据文件中，如所示。 使用选项卡可分隔UUID和特征ID。 文件中不需要<code> ic </code>前缀。 </p> <p><b>数字ID</b> </p> <p> 
     <code>
...
下图显示了正确格式入站文件方式的示例，具体取决于[ID类型](../../../reference/ids-in-aam.md)和要向用户档案添加[!UICONTROL traits]的方法。
...
使用[!UICONTROL trait IDs]发送[!DNL Audience Manager] [!DNL UUIDs]的[!UICONTROL trait]鉴别信息。
...
使用[!UICONTROL trait IDs]发送[!UICONTROL trait] [!DNL Audience Manager] [!DNL UUIDs]的取消资格信息。
...
发送键值对，以添加[!DNL Audience Manager] [!DNL UUIDs]的[!UICONTROL trait]鉴别信息。
...
使用`ic`前缀发送[!UICONTROL trait] [!DNL Audience Manager] [!DNL UUIDs]的资格信息。
...
使用[!UICONTROL trait IDs]发送[!UICONTROL trait]设备的[!DNL Android]鉴别信息。
...
使用[!UICONTROL trait IDs]发送[!UICONTROL trait]设备的取消资格信息。[!DNL Android]
...
发送键值对，以添加[!UICONTROL trait]设备的[!DNL Android]鉴别信息。
...
使用`ic`前缀发送[!UICONTROL trait]设备的资格信息。[!DNL Android]
...
使用[!UICONTROL trait IDs]发送[!UICONTROL trait]设备的[!DNL iOS]鉴别信息。
...
使用[!UICONTROL trait IDs]发送[!UICONTROL trait]设备的取消资格信息。[!DNL iOS]
...
发送键值对，以添加[!UICONTROL trait]设备的[!DNL iOS]鉴别信息。
...
使用`ic`前缀发送[!UICONTROL trait]设备的资格信息。[!DNL iOS]
...
发送键值对，以添加[!UICONTROL trait]的[!DNL DPUUIDs]鉴别信息。
...
使用`ic`前缀发送[!UICONTROL trait]的[!DNL DPUUIDs]鉴别信息。
