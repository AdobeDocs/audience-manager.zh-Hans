...
* 有关 [!UICONTROL Google Campaign Manager] 日志，请参 [阅将Google活动管理器](../../reporting/audience-optimization-reports/aor-advertisers/import-dcm.md) 文件导入Audience Manager并联 *系您* 的顾 [!DNL Audience Manager] 问。
...
### Google活动管理器日志中的可操作信号 {#dcm-logs-signals}

此表列表日志文件中的可操 [!DNL Google Campaign Manager] 作信号：
...
<table id="table_A5A2A10D471C4C9D8DCD88F9C017040C"> 
 ...
 <tbody> 
  <tr> 
   ...
   <td colname="col3"> <p>仅适用于转化事件。 </p> <p>表示Google活动管理器中转换活动的数字ID。 此字段映射到Google活动管理器中的活动ID。 </p> <p> <p>提示： 您可以从Google活动管理器中捕获多个或特定的转化活动。 使用Google活动 <code> d_conversion = activity ID</code> 管理器中的每个转换活动创建特征。 </p> </p> </td> 
  ...
  <tr> 
   ...
   <td colname="col3"> <p>用于捕获Google活动管理器数据的数据源的ID。 了 <a href="../../features/manage-datasources.md#create-data-source"> 解如何创建数据源</a>。 </p> </td> 
   <td colname="col4"> <p> <code> 743</code> </p> </td> 
  </tr>
 </tbody>
</table>

表中描述的信号像实 [!DNL Audience Manager] 时呼叫一样被捕 `HTTP` 获。 以下示例调用包含有关转换事件的信息 [!DNL Google Campaign Manager]。 调用不一定必须包含 *示例* 调用中的所有信号。

```
https://yourcompany.demdex.net?d_src=743&d_uuid=07955261652886032950143702505894272138&d_time=1504536233&d_event=conv&d_conversion=24122&d_conversionType=2&d_bu=3983524&d_campaign=7321391&d_adsrc=11111&d_creative=123456
```

对于平均大小为200万 [!DNL Google Campaign Manager] 行的日志文件，通过可操作信号创建的任何特征在我们处理日志后大约一小时内即可实现。
...
>[!NOTE]
>
>日志中提供的事件 [!DNL Google Campaign Manager] 时间戳将被接受并传递给 [!UICONTROL Data Collection Servers]。
>
>* 如果日志文件中的数据行没有时间戳 [!DNL Google Campaign Manager] 可用，我们将调用的时间 `HTTP` 用作事件时间戳。
>* 如果日志文件中的数 [!DNL Google Campaign Manager] 据行包含格式错误的时间戳，我们将忽略整行。
...
### 在活动优化报告或受众实验室中使用Google受众管理器Floodlight活动

[Floodlight标签使广告商](https://support.google.com/dcm/partner/answer/4293719?hl=en) 能够跟踪用户转化率。 您可 [!UICONTROL Actionable Log Files]以在受众优化 [!DNL Google Campaign Manager] 报告或受众实 [验室中跟](../../reporting/audience-optimization-reports/audience-optimization-reports.md) 踪转 [化](../../features/audience-lab/audience-lab.md)率：
