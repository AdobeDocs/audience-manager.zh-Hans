...
<pre class="java"><code>
var my_object = { color:“blue”，价格："900" };
 
var sample_dil = DIL.create({ partner :"<i>partner name</i>" });
//加载对象，将“c_”附加到键值对中的所有键，并将数据发送到AudienceManager。 
...
<pre class="java"><code>
var my_objects = [{ color:“blue”，价格："900" }, { type :“acura”，模型："tl" }];
 
var sample_dil = DIL.create({ partner :"<i>partner name</i>" });
 
for(var i = 0;i &lt; my_objects.length;i++)//加载对象，并将“c_”附加到键值对中的所有键。  
{ sample_dil.api.signs(my_objects[i], "c_");
} sample_dil.api.submit();
</code></pre>
